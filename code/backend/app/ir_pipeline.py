...
def find_similar_cases(
    ...
    return [{"case_id": case_id, "similarity": round(float(sim), 4), "rank": rank + 1} for rank, (case_id, sim) in enumerate(top.items())]
...
def extract_key_sentences(
    ...
    top_indices = sorted(np.argsort(combined_scores)[-top_n:].tolist())  # 원문 순서 유지
