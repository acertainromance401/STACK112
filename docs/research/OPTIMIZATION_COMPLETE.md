...
아래 표는 텍스트 처리와 직접 연결된 성능 개선을 날짜순으로 압축한 요약이다.

| 시점 | 핵심 변경 | 수치/정량 효과 |
|---|---|---|
| 2026-05-06 | OCR, JSON 디코딩, LLM 추론을 메인 스레드에서 분리 | OCR 블로킹 5-10초 제거, JSON 디코딩 CPU 1-2% 부담 제거 |
| 2026-05-06 | LLM 입력/출력 크기 축소 | 프롬프트 1200자 제한, 생성 토큰 512 → 160 (69% 감소) |
| 2026-05-06 | 전체 CPU/메모리 최적화 1차 완료 | CPU 88.2% → 60% (32% 감소), 메모리 213MB → 120MB (44% 감소) |
| 2026-05-07 | 정형화 출력 검증 안정화 | 파싱 성공률 95%+ |
| 2026-05-11 | OCR 원문 정규화 및 문장 품질 보정 | leading particle 제거, 헤더 정리, 핵심 문장 3-5개 유지 |
